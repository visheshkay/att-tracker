...
function Register() {
    ...
    async function registerUser(user) {
        console.log(user);
        try {
          const res = await axios.post("http://127.0.0.1:8000/students/register", user);
      
          console.log(res.data);
      
          if (res.data.status === "registered") {
            // FastAPI returns { roll: ..., status: "registered" }
            navigate("/login");
          } else {
    ...
    <div className="facid mb-4">
            ...
            <input type="text" id="facid" className="form-control w-50 mx-auto m-3" placeholder="StudentId" {...register("roll",{required:true,minLength:4,maxLength:10})}/>
            ...
            <input type="text" id="uname" className="form-control w-50 mx-auto m-3"  placeholder="Username"{...register("name",{required:true,minLength:4,maxLength:20})}/>
